...
void main()
{
	...
	diff = max(dot(lightdir, normal), 0.0); // Diffuse lighting
	
	
	vec3 viewDir = normalize(fragPos - vPosition); // Calculate the view direction
	vec3 reflectDir = reflect(lightdir, normal); // Calculate the reflected light direction
	float specAngle = max(dot(viewDir, reflectDir), 0.0); // Calculate the specular angle

	spec = pow(specAngle, phongExp); // Specular lighting using Phong exponent
